...
curl -H "Content-Type: application/json" -d '{"sessionID": "'$session_Id'","masterKey": "'$master_Key'","sampleMessage": "Hi server, please encrypt me and send to client!"}' -X POST 'http://16.16.53.16:8080/k>jq -r '.encryptedSampleMessage' response_message.json | base64 -d > encSampleMsgReady.txt.enc
