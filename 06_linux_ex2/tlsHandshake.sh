#! /bin/bash
echo  "welcome to  eli's handshake!!!"
curl -# -o 'response.json' -H "Content-Type: application/json" -d '{"clientVersion": "3.2", "message": "Client Hello"}'>sessionId=$(jq -r '.sessionID' response.json)
...
curl -# -o 'response_message.json' -H "Content-Type: application/json" -d '{"sessionID": "'$sessionId'","masterKey": "'>jq -r '.encryptedSampleMessage' response_message.json | base64 -d >encSampleMsgReady.txt
